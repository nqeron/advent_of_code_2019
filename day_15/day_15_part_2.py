...
def next_pos_is_empty(terrain, pos, direction: Direction) -> bool:
    ...
    if direction is Direction.NORTH:
        ...
        x, y = x + 1, y
    return terrain[(x, y)] is Tile.Empty
...
def analyze(file):
    ...
    count = 0
    while sum([1 if terrain[pos] is Tile.Empty else 0 for pos in terrain]) > 0:
        count += 1
        seen = set()
        for pos in terrain:
            if terrain[pos] is not Tile.Oxygen or pos in seen:
                continue
            x, y = pos
            up = (x, y - 1)
            down = (x, y + 1)
            ...
            for new_pos in [up, down, left, right]:
                if terrain[new_pos] is Tile.Empty:
                    terrain[new_pos] = Tile.Oxygen
                    seen.add(new_pos)
        print_terrain(terrain, (1000, 1000), Direction.SOUTH)
        print(count)
    print(count)
